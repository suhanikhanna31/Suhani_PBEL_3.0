...
#
# contact_degree / interaction_concentration added from
# src/features/network_features.py (src/dsa/communication_graph.py under
# the hood): the only two tracked features derived from message *headers*
# (who someone talks to) rather than message *content* (what/how they
# write). Tracked here rather than in a parallel scoring path so a
# shrinking, more concentrated communication circle drifts through the
# exact same sliding-window baseline + weighted z-score machinery as every
# linguistic feature, instead of needing its own bespoke threshold logic.
TRACKED_FEATURES = [
